...
class AppointmentService {
    ...
    async AppointmentPatientDoctor(doctorId) {
        const q = query(collection(db, "appointments"), where("doctorId", "==", doctorId),where("appointmentStatus","in",["Pending","Accepted","Completed"]))
        const querySnapshot = await getDocs(q)
        let appointments = []
        querySnapshot.forEach((appt) => {
            // doc.data() is never undefined for query doc snapshots
